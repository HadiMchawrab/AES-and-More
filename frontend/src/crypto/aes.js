// Pure-JS AES-128/192/256 single-block encrypt/decrypt.
// Implements the Rijndael spec (FIPS 197). Used only as the building block for
// the mode implementations in modes.js; the modes wire up ECB/CBC/CFB/OFB/CTR
// around this primitive.

// S-box
const SBOX = new Uint8Array([
  ,
]);

const INV_SBOX = new Uint8Array(256);
for (let i = 0; i < 256; i++) INV_SBOX[SBOX[i]] = i;

const RCON = new Uint8Array([
  0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
]);

// GF(2^8) multiply by 2 with reduction by x^8 + x^4 + x^3 + x + 1 (0x1b).
function xtime(b) {
  return ((b << 1) ^ ((b & 0x80) ? 0x1b : 0)) & 0xff;
}

function gmul(a, b) {
  let p = 0;
  for (let i = 0; i < 8 && a && b; i++) {
    if (b & 1) p ^= a;
    a = xtime(a);
    b >>= 1;
  }
  return p & 0xff;
}

// Key expansion. Returns a Uint8Array of length 16 * (Nr + 1).
export function expandKey(key) {
  const keyLen = key.length;
  if (keyLen !== 16 && keyLen !== 24 && keyLen !== 32) {
    throw new Error(`Invalid AES key length: ${keyLen} bytes (must be 16, 24, or 32)`);
  }
  const Nk = keyLen / 4;
  const Nr = Nk + 6;
  const totalWords = 4 * (Nr + 1);
  const W = new Uint8Array(totalWords * 4);

  W.set(key);

  for (let i = Nk; i < totalWords; i++) {
    const off = i * 4;
    let t0 = W[off - 4];
    let t1 = W[off - 3];
    let t2 = W[off - 2];
    let t3 = W[off - 1];

    if (i % Nk === 0) {
      // RotWord
      const a = t0;
      t0 = t1; t1 = t2; t2 = t3; t3 = a;
      // SubWord
      t0 = SBOX[t0]; t1 = SBOX[t1]; t2 = SBOX[t2]; t3 = SBOX[t3];
      // XOR with Rcon
      t0 ^= RCON[i / Nk];
    } else if (Nk > 6 && i % Nk === 4) {
      t0 = SBOX[t0]; t1 = SBOX[t1]; t2 = SBOX[t2]; t3 = SBOX[t3];
    }

    W[off]     = W[off - Nk * 4]     ^ t0;
    W[off + 1] = W[off - Nk * 4 + 1] ^ t1;
    W[off + 2] = W[off - Nk * 4 + 2] ^ t2;
    W[off + 3] = W[off - Nk * 4 + 3] ^ t3;
  }
  return W;
}

function addRoundKey(state, roundKey, offset) {
  for (let i = 0; i < 16; i++) state[i] ^= roundKey[offset + i];
}

function subBytes(state) {
  for (let i = 0; i < 16; i++) state[i] = SBOX[state[i]];
}

function invSubBytes(state) {
  for (let i = 0; i < 16; i++) state[i] = INV_SBOX[state[i]];
}

// State is column-major: state[r + 4*c] = byte at row r, column c.
function shiftRows(state) {
  let t;
  // row 1: left 1
  t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
  // row 2: left 2
  t = state[2]; state[2] = state[10]; state[10] = t;
  t = state[6]; state[6] = state[14]; state[14] = t;
  // row 3: left 3 (= right 1)
  t = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;
}

function invShiftRows(state) {
  let t;
  // row 1: right 1
  t = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = t;
  // row 2: right 2
  t = state[2]; state[2] = state[10]; state[10] = t;
  t = state[6]; state[6] = state[14]; state[14] = t;
  // row 3: right 3 (= left 1)
  t = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = t;
}

function mixColumns(state) {
  for (let c = 0; c < 4; c++) {
    const i = c * 4;
    const s0 = state[i], s1 = state[i + 1], s2 = state[i + 2], s3 = state[i + 3];
    state[i]     = xtime(s0) ^ (xtime(s1) ^ s1) ^ s2 ^ s3;
    state[i + 1] = s0 ^ xtime(s1) ^ (xtime(s2) ^ s2) ^ s3;
    state[i + 2] = s0 ^ s1 ^ xtime(s2) ^ (xtime(s3) ^ s3);
    state[i + 3] = (xtime(s0) ^ s0) ^ s1 ^ s2 ^ xtime(s3);
  }
}

function invMixColumns(state) {
  for (let c = 0; c < 4; c++) {
    const i = c * 4;
    const s0 = state[i], s1 = state[i + 1], s2 = state[i + 2], s3 = state[i + 3];
    state[i]     = gmul(s0, 0x0e) ^ gmul(s1, 0x0b) ^ gmul(s2, 0x0d) ^ gmul(s3, 0x09);
    state[i + 1] = gmul(s0, 0x09) ^ gmul(s1, 0x0e) ^ gmul(s2, 0x0b) ^ gmul(s3, 0x0d);
    state[i + 2] = gmul(s0, 0x0d) ^ gmul(s1, 0x09) ^ gmul(s2, 0x0e) ^ gmul(s3, 0x0b);
    state[i + 3] = gmul(s0, 0x0b) ^ gmul(s1, 0x0d) ^ gmul(s2, 0x09) ^ gmul(s3, 0x0e);
  }
}

export function aesEncryptBlock(block, key) {
  if (block.length !== 16) throw new Error('Block must be 16 bytes');
  const W = expandKey(key);
  const Nr = W.length / 16 - 1;
  const state = new Uint8Array(block);

  addRoundKey(state, W, 0);
  for (let round = 1; round < Nr; round++) {
    subBytes(state);
    shiftRows(state);
    mixColumns(state);
    addRoundKey(state, W, round * 16);
  }
  subBytes(state);
  shiftRows(state);
  addRoundKey(state, W, Nr * 16);
  return state;
}

export function aesDecryptBlock(block, key) {
  if (block.length !== 16) throw new Error('Block must be 16 bytes');
  const W = expandKey(key);
  const Nr = W.length / 16 - 1;
  const state = new Uint8Array(block);

  addRoundKey(state, W, Nr * 16);
  for (let round = Nr - 1; round > 0; round--) {
    invShiftRows(state);
    invSubBytes(state);
    addRoundKey(state, W, round * 16);
    invMixColumns(state);
  }
  invShiftRows(state);
  invSubBytes(state);
  addRoundKey(state, W, 0);
  return state;
}

export function xorBytes(a, b) {
  const n = Math.min(a.length, b.length);
  const out = new Uint8Array(n);
  for (let i = 0; i < n; i++) out[i] = a[i] ^ b[i];
  return out;
}
