...
import { DataBox, AESBox, XORCircle, KeyArrow, Arrow, DiagramDefs } from './DiagramPrimitives';

/**
 * CTR Mode Flow Diagram
 *
 *   ┌─────────────────────┐   (dashed: parallelizable)
 *   │  Counter i          │
 *   │      ↓              │
 *   │ K → [Encrypt]       │
 *   └─────────────────────┘
...
function CTRDiagram({ blocks, isEncrypt, animatedUpTo }) {
  ...
  const Y = {
    ...
    dashBot: 124,
    ksLabel: 126,
    xorY: 152,     // 28px gap below AES
    dataY: 134,
    outY: 184,     // 18px gap below XOR
    outBot: 220,
  };

  function renderBlock(block, i, cx, active, d) {
    const counterValue = block.counter_value !== undefined ? block.counter_value : i;
    const counterHex = block.counter_bytes;
    ...
    return (
      <g key={i}>
        ...
        {/* Counter */}
        <DataBox x={cx - 55} y={Y.ctrY} w={110} label={`Counter ${counterValue}`}
          value={counterHex} color="#e3d4a0" active={active} delay={d} />

        {/* Arrow: counter down to AES */}
        <Arrow x1={cx} y1={Y.ctrBot} x2={cx} y2={Y.aesY}
